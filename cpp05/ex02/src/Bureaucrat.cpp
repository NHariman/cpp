...
/*   Updated: 2022/03/21 21:01:10 by nhariman      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void		Bureaucrat::executeForm(Form &obj)
{
	try
	{
		std::cout << this->_name << " has been approved to execute form: " << obj.getName() << std::endl;
		obj.execute(*this);
	}
	catch (const std::exception& e)
	{
		std::cout << this->_name << " could not executed form " << obj.getName() << " because " << e.what() << std::endl;
	}
}
